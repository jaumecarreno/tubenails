...
export const DashboardLayout = ({ children }: { children: React.ReactNode }) => {
    ...
    if (loading) {
        return null; // Wait for hydration and auth state
    }
                ...
                <div className="p-4 border-t border-slate-200 dark:border-slate-700 space-y-4">
                    ...
                    <div className="flex items-center gap-3 px-2">
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-slate-900 dark:text-white truncate">{user.user_metadata.full_name || 'Usuario'}</p>
                            <p className="text-xs text-slate-500 dark:text-slate-400 truncate">{user.email}</p>
                        </div>
                        <button onClick={handleLogout} title="Cerrar Sessión" className="flex-shrink-0 text-slate-400 hover:text-red-500 transition-colors">
